...
## Resultados de validación (5-fold cross-validation out-of-fold)

| Target | OOF MAE | OOF R² | vs Linear baseline |
|--------|---------|--------|-------------------|
| people_affected_7d | 9.63 ± 1.06 | 0.990 | **−81.8%** error |
| people_affected_30d | 28.85 ± 2.28 | 0.990 | **−82.2%** error |
| fish_dieoff_pct | 1.43 ± 0.04 | 0.992 | **−76.9%** error |
| recovery_days | 4.47 ± 0.27 | 0.989 | **−32.3%** error |

**Validación adicional:**
- **8/8 smoke tests** pasan en pytest (2.5 s)
- **4 de 5 casos reales documentados** (Cuninico 2014, Pluspetrol Lote 8, Lote 192, turbidez, OOD) producen predicciones en rango esperado por Oxfam/Mongabay
- **Sin sesgo por subgrupo** (analizado en 12 subgrupos: tipos × estaciones × volúmenes; |bias| < 10% en todos)

---

## Feature importance (lo que el modelo aprendió del dominio)

- **`people_affected_*`:** `volume_barrels` domina al 83% → derrame grande = más gente afectada. ✓ Coherente.
- **`fish_dieoff_pct`:** `contamination_type_enc` domina al 81% → hidrocarburo mata más que turbidez. ✓ Coherente.
- **`recovery_days`:** mix `volume_barrels` (54%) + `contamination_type` (42%) → ambos importan. ✓ Coherente.

Si `season_enc` hubiera dominado fish_dieoff, sería sospechoso. Como aprendió relaciones físicamente correctas, **no está sobreajustando ruido**.

---

## Lo que el modelo NO afirma

1. **No mide contaminación.** Predice impacto downstream dado una contaminación ya detectada (Capa DETECT lo hace via Claude Vision).
...
4. **No es ground truth de OEFA.** OEFA no publica counts reales — los targets son modelados.
...
- [`v1/backend/ml/metrics.json`](_docx_extract/v1/backend/ml/metrics.json) — CV, baselines
